  ...
  describe("Anonymous user", () => {
    test("With nonexistent token", async () => {
      const response = await fetch(
        "http://localhost:3000/api/v1/activations/41c0544d-11cc-4ec6-b652-cea39dcf9636",
        {
          method: "PATCH",
        },
      );
    ...
    test("With expired token", async () => {
      jest.useFakeTimers({
        now: new Date(Date.now() - activation.EXPIRATIONS_IN_MILLISECONDS),
      });
    ...
    test("With valid token", async () => {
      ...
      expect(expiresAt - createdAt).toBe(
        activation.EXPIRATIONS_IN_MILLISECONDS,
      );
